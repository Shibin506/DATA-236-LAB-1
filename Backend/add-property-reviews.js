...
async function addPropertyReviews() {
  try {
    ...
    console.log(`📝 Found ${properties.length} properties to add reviews for`);
    console.log(`👥 Found ${travelers.length} travelers for reviews`);
    ...
    for (const property of properties) {
      ...
      if (reviews.length > 0) {
        console.log(`📝 Adding ${reviews.length} reviews for: ${propertyName}`);
        
        for (const review of reviews) {
          // Select a random traveler for each review
          const randomTraveler = travelers[Math.floor(Math.random() * travelers.length)];
          
          // Create a fake booking_id (we'll use a simple increment)
          const bookingId = totalReviews + 1;
        ...
        // Add generic reviews for properties without specific templates
        ...
        console.log(`📝 Adding generic reviews for: ${propertyName}`);
    ...
    console.log('✅ Successfully added property reviews');
    console.log(`📊 Summary:`);
    console.log(`   - Total properties: ${properties.length}`);
    console.log(`   - Total reviews added: ${totalReviews}`);
    console.log(`   - Each property has 1-3 reviews`);
